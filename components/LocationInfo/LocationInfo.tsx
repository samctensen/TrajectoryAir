    ...
    queryFn: async () => {
      const response = await fetch(
        `https://api.mapbox.com/search/geocode/v6/reverse?longitude=${
          latLng![1]
        }&latitude=${latLng![0]}&access_token=${
          ...
          try {
            const response = await fetch(
              `https://api.mapbox.com/v4/${
                process.env.NEXT_PUBLIC_MAPBOX_USERNAME
              }.${id}/tilequery/${latLng![1]},${
                latLng![0]
              }.json?radius=6000&limit=1&access_token=${
                process.env.NEXT_PUBLIC_MAPBOX_TOKEN
              }`
            );
            if (!response.ok) {
    ...
    <div className="location-info slide-in">
      ...
      <div className="text-white mt-4 ml-3">
        <h3 className="text-2s font-bold text-white">
          {Math.abs(latLng![0]).toFixed(2)}° {latLng![0] > 0 ? "N" : "S"},{" "}
          {Math.abs(latLng![1]).toFixed(2)}° {latLng![1] > 0 ? "E" : "W"}
        </h3>
      </div>
      {address != undefined && (
        <div
          className={`text-white ml-3 ${address != undefined ? "mt-3" : ""}`}
        >
      ...
      <div className="text-white mt-3 ml-3">
        <h3 className="text-2s font-bold text-white">
          PM-2.5:{" "}
          {graphData.data[24 * sliderDateIndex + sliderTime]?.y === 0 ||
          graphData.data[24 * sliderDateIndex + sliderTime] === undefined
            ? "---"
            : graphData.data[24 * sliderDateIndex + sliderTime]?.y}
        </h3>
      </div>
      <div className="text-white mt-3 ml-3">
        <h3 className="text-2s font-bold text-white">
      ...
      <button
        className="location-info-share-button"
        onClick={async () => {
          try {
